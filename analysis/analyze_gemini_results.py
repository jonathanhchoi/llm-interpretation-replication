...
def analyze_gemini_model(df, model_name, prompts, output_dir):
    """Perform complete analysis for Gemini model (adapted for confidence-only data)."""
    ...
    if all_tables:
        with open(output_dir / 'all_tables.tex', 'w') as f:
            f.write("% All LaTeX tables for Gemini analysis\n\n")
...
if __name__ == "__main__":
    # Configuration for Gemini results
    GEMINI_RESULTS_FILE = Path("G:/My Drive/Colab Notebooks/llm_interpretation_replication/results/gemini_perturbation_results.xlsx")
    OUTPUT_DIR = Path("G:/My Drive/Colab Notebooks/llm_interpretation_replication/results/gemini_analysis")

    # Check if the results file exists
    if not GEMINI_RESULTS_FILE.exists():
        print(f"ERROR: Results file not found at {GEMINI_RESULTS_FILE}")
        print("Please ensure the Gemini perturbation results file exists.")
        exit(1)

    print(f"Reading Gemini results from: {GEMINI_RESULTS_FILE}")
    df = pd.read_excel(GEMINI_RESULTS_FILE)
    ...
    if 'Model' in df.columns:
        ...
        model_name = models[0] if len(models) == 1 else "Gemini_Combined"
    else:
        model_name = "gemini-2.0-flash"  # Default based on the script
    ...
    # Perform analysis
    print("\n" + "="*60)
    print("STARTING GEMINI CONFIDENCE ANALYSIS")
    print("="*60)

    analyze_gemini_model(df, model_name, prompts, OUTPUT_DIR)
